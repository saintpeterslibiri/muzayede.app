...
async function getBidsByAuction(req, res) {
    try {
        ...
        const [bids] = await db.pool.query(sql, [auctionId]);
...
async function placeBid(req, res) {
    try {
        ...
        const [result] = await db.pool.query(insertSql, [auctionId, userId, bidAmount]);
        ...
        const [updatedAuction] = await db.pool.query(
            'SELECT current_price FROM auctions WHERE id = ?',
            [auctionId]
        );
        
        // Get the newly created bid
        const [newBid] = await db.pool.query(
            `SELECT b.*, u.username 
             FROM bids b 
             JOIN users u ON b.user_id = u.id 
             WHERE b.id = ?`,
...
async function setAutoBid(req, res) {
    try {
        ...
        await db.pool.query(upsertSql, [auctionId, userId, maxAmount, maxAmount]);
...
async function processAutoBids(auctionId, excludeUserId, currentBidAmount) {
    try {
        ...
        const [autoBids] = await db.pool.query(sql, [auctionId, excludeUserId, currentBidAmount]);
        ...
        const [remainingAutoBids] = await db.pool.query(
