...
async function getStats(req, res) {
    try {
        ...
        // Users by status
        const [usersByStatus] = await db.query(
        ...
        // Total auctions
        const [totalAuctions] = await db.query(
            'SELECT COUNT(*) as count FROM auctions'
        );
        
        // Auctions by status
        const [auctionsByStatus] = await db.query(
            `SELECT status, COUNT(*) as count 
             FROM auctions 
             GROUP BY status`
        );
        ...
        const [auctionsByCategory] = await db.query(
        ...
        const [auctionsThisMonth] = await db.query(
            `SELECT COUNT(*) as count 
             FROM auctions 
             WHERE created_at >= DATE_FORMAT(NOW(), '%Y-%m-01')`
        );
        ...
        // Bids this month
        const [bidsThisMonth] = await db.query(
            `SELECT COUNT(*) as count 
             FROM bids 
             WHERE created_at >= DATE_FORMAT(NOW(), '%Y-%m-01')`
        );
        ...
        const [avgBidsPerAuction] = await db.query(
        ...
        const [totalActiveValue] = await db.query(
        ...
        const [totalSalesValue] = await db.query(
            `SELECT SUM(current_price) as total 
             FROM auctions 
        ...
        const [recentAuctions] = await db.query(
